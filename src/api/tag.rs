...
pub async fn create_tag(
    ...
    let exist_tag = Tag::find_by_name(&state.db, &tag_info.name).await?;
    ...
    let uid = Tag::create(&state.db, &tag_info).await?;
    let new_tag = Tag::find_by_id(&state.db, uid as i32).await?;
    if new_tag.is_none() {
...
pub async fn get_tags(
    ...
    let tags = Tag::find_list(&state.db, &pagination).await?;
...
pub async fn update_tag(
    ...
    let update_ok = Tag::update(&state.db, id, &tag_info).await?;
    if !update_ok {
        return Err(Error::NotFound(String::from("tag")));
    }
